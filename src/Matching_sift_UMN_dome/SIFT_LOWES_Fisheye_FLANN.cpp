...
void Iterate_SIFT_STATIC_MP(vector<FrameCamera> &vFC, int currentFC, vector<Feature> &feature_static);
using namespace std;
...
int main ( int argc, char * argv[] )
{
	...
	for (int iFrame = 0; iFrame < vCamera[0].vTakenFrame.size(); iFrame++)
	{
		...
		fc.imageFileName[fc.imageFileName.size()-3] = 'j';
		fc.imageFileName[fc.imageFileName.size()-2] = 'p';
		fc.imageFileName[fc.imageFileName.size()-1] = 'g';
	...
	for (int iFC = 0; iFC < vFrameOrder.size(); iFC++)
	{
		...
		Iterate_SIFT_STATIC_MP(vFC, iFC1, feature_static);
