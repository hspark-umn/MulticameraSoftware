...
vector < ArvCamera* > camera;

// vector<int> nFrameIds;

static gboolean bReady2Send = false;
...
void sendTcp(int buf_id, JpgData* imgbuf, string cam_id) // vector<JpgData>&
{
    while(!cancel)
    {
        {
            ...
            if(is_buf_readable(id_BufRd[buf_id], id_BufWr[buf_id]))
            {
                ...
                // char temp[1000];
                // int nLen = int(imgbuf[id_BufRd[buf_id]].img_jpg.size());
                // sprintf(temp, "/home/odroid/img%07d-%s.jpg", imgbuf[id_BufRd[buf_id]].img_id, cam_id.c_str());
                // FILE *f = fopen(temp, "wb");
                // fwrite(imgbuf[id_BufRd[buf_id]].img_jpg.data(), 1, nLen, f);
                // fclose(f);

// cout << "cid=" << cam_id << "pid=" << imgbuf[id_BufRd[buf_id]].img_id << " to be sent...\n";                
                tcp_filetransfer(imgbuf[id_BufRd[buf_id]].img_id, tcpFd[buf_id], cam_id, imgbuf[id_BufRd[buf_id]].img_jpg);
	...
	if (buffer != NULL) {
		...
		/* Image processing here */
            ...
            if(bReady2Send)
            {
                ...
                // Size ds(width/2, height/2);          
                // resize(image, image, ds);            
...
periodic_task_cb (void *abstract_data)
{
    ...
    if(!bReady2Send)
    {
        // Wait for 1s to clear possible history images first.
        bReady2Send = true;
...
int main (int argc, char **argv)
{
    ...
    camera.resize(nCam);// vector < ArvCamera* > camera(nCam);
    ...
    {   
        ...
        cout << "Connecting TCP " << i << "\n";
        if(connectServer(tcpFd[i]) < 0)
        {
            for(int j=0; j <= i; j++)
            {
                close(tcpFd[j]);
        ...
        {
      //       // Clear history images first.
      //       ArvBuffer *buffer;
      //       cout << "Clearing buffer......";
		    // while(1)
      //       {
      //           buffer = arv_camera_acquisition(camera[i], 10);
...
// sendTcp(int buf_id, JpgData* imgbuf, string cam_id)            
            vt.push_back(thread(sendTcp, i, imgBuffer[i].data(), data[i].cam_id));            
    ...
    for(i = 0; i < nCam; i++)
    {
	    ...
	    g_signal_connect (arv_camera_get_device (camera[i]), "control-lost", G_CALLBACK (control_lost_cb), NULL);
    }
    ...
    // To clear history buffer, the camera cannot be started now, 1s later instead.
    // for(i = nCam-1; i >= 0; i--)
    // { 
    // 	// Start the video stream
	   //  arv_camera_start_acquisition (camera[i]);
